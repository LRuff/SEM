...
```{r likelihood convergence, fig.cap = "Convergence of observed log likelihood. Increase in steps corresponds to increase in observed log likelihood. \\label{fig:likelihood-convergence}", echo = FALSE}
# Plot convergence of log likelihood
plot(result$loglike, type = "l", xlab = "Number of EM iterations", ylab = "Log-Likelihood")
grid()
```

```{r contour plot, fig.height = 6, fig.cap = "Contour plot of EM algorithm. Iteration 0 corresponds to the initial condition.\\label{fig:contour}", echo = FALSE}
# Contour plot
par(mfrow=c(2,2))
...
In case of MAR, the problems with calculation of $V$ using SEM are more clear, but still the values are not too far away from the MC results, which can also be seen in \ref{fig:fig:sd-mar}.
