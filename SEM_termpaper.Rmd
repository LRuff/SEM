...
In our implementation of the SEM algorithm, which can be found in the .Rmd-file, unfortunately we still receive a non symmetric $\Delta V$. We obtain the correct value for $\mu_2$ in $DM$, but because of the different parameterization, we cannot compare the other values directly. The sources of error could be two-fold: (1) the implementation (or formula derivation) of the inverse Fisher information matrix is not correct or (2) the computation of $DM$ has coding errors.

As emphasized in the paper, one nice feature of SEM is, that coding errors or numerical errors can be diagnosed from the symmetry of $\Delta V$, which unfortunately in our case, is still asymmetric.
...
To compare the results of the SEM variance estimation with some other estimation, a simulation study was implemented. The simulation is based on @enders2010missing chapter 4.8. Enders used two variables which are normally distributed. The first variable $X$ was chosen as intelligence with $\mu_X = 100$ and $\sigma_{XX} = 169$. The second variable $Y$, job perfornance, follows a ditribution with $\mu_Y = 12$ and $\sigma_{YY} = 9$. The covariance between them should be $\sigma_{XY} = 19.5$. $X$ was completely observed, where $Y$ was partly missing. In case of MCAR 50% of all dataset entries were removed, in case of MAR thos performance values were removed, were the intelligence was lower than median.

Like Enders, we simulated $1000$ datasets, each having a size of $N = 250$. We obtained the variances, using SEM algorithm and MC estimation. Using the SEM algorithm, we obtained a variance estimation for every dataset, taking the quare rooted diagonal to get standard deviations. Finally the mean over all variance estimations was taken. In MC simulation, the ML estimators were calculated for every sample, finally the standard deviation of the estimators were calculated.
...
The results for MC and SEM are very similar for $\mu_y$ and $\sigma_{yy}$ in case of MCAR. The SEM result of $\sigma_{xy}$ was not very close to the result of the MC estimation, which is probably related to the problems we had, calculating the SEM variances. In the plot \ref{fig:fig:sd-mcar}, the differences in standard deviation for $\mu_y$ is shown.

```{r mcar plots, echo = FALSE, fig.cap = "Standard deviation for mu_y using SEM and MC for MCAR data\\label{fig:sd-mcar}"}
# Plot results
plotSDMu2(results_mcar, nSim)
```

```{r mar ana, cache = TRUE}
# Generate MAR data
...
```{r mar plots, echo = FALSE, fig.cap = "Standard deviation for mu_y using SEM and MC for MAR data\\label{fig:sd-mar}"}
# Plot results
